...
const SidebarItem = ({ to, icon, label, filled = false }: { to: string; icon: string; label: string; filled?: boolean }) => {
  return (
    ...
    >
      {({ isActive }) => (
        <>
          <span className={`material-symbols-outlined ${isActive || filled ? 'icon-filled' : ''}`}>{icon}</span>
    ...
    <div className="flex h-screen w-full overflow-hidden relative">
      ...
      <aside className="w-20 lg:w-64 flex-shrink-0 flex flex-col justify-between border-r border-night-border bg-night-sidebar/95 backdrop-blur-md z-30">
        <div className="flex flex-col gap-6 p-4">
          {/* Logo Section */}
          <div className="flex items-center gap-3 pb-6 border-b border-night-border">
        ...
        <div className="w-full bg-night-panel/80 backdrop-blur-md border-t border-night-border py-3 px-6 flex justify-center items-center z-10 flex-shrink-0">
            <div className="flex items-center gap-2 text-[11px] tracking-widest text-slate-500">
                ...
                <span className="ml-1 flex items-baseline gap-1 opacity-80 hover:opacity-100 transition-opacity">
                    <span className="lowercase italic font-serif">by</span>
                    <span className="uppercase italic font-bold text-slate-400">FLAVIO JR.</span>
                </span>
            </div>
        </div>
      </main>
