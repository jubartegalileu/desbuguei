...
export const Settings = () => {
  ...
  const handleUnlockAdmin = (e: React.FormEvent) => {
      e.preventDefault();
      // Use environment variable for password, fallback to 'admin' for dev/test
  ...
  const handleSeedDatabase = async (useCustom: boolean = false) => {
    if (seeding) return;
    setSeeding(true);
    ...
    if (useCustom) {
        ...
        if (list.length === 0) {
            setSeedLogs(prev => [...prev, "❌ Lista vazia. Digite alguns termos."]);
            setSeeding(false);
            return;
        }
    }
    ...
    setSeeding(false);
    if (useCustom) setCustomListText(""); 
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 lg:p-12 pb-32">
      ...
      <div className="flex flex-col gap-12">
        ...
        <section className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
            <div className="flex items-center gap-3 pb-4 border-b border-night-border/50">
                ...
                {!isAdminUnlocked && (
                     <span className="ml-auto flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-slate-500 border border-slate-700 rounded px-2 py-0.5">
            ...
            {!isAdminUnlocked ? (
                // --- LOCKED STATE ---
                <div className="glass-card rounded-2xl p-12 border-night-border bg-night-panel/30 flex flex-col items-center text-center gap-6">
                    <div className="size-16 rounded-full bg-night-bg border border-night-border flex items-center justify-center text-slate-500 shadow-inner">
                        <span className="material-symbols-outlined text-3xl">shield_lock</span>
                    </div>
                    <div>
                        <h4 className="text-lg font-bold text-slate-200 mb-2">Acesso Administrativo Necessário</h4>
                        <p className="text-slate-400 text-sm max-w-md mx-auto">
                            A alimentação do banco de dados com Inteligência Artificial é restrita para evitar custos excessivos de API e manutenção da integridade dos dados.
                        </p>
                    </div>
                ...
                // --- UNLOCKED STATE ---
                <div className="glass-card rounded-2xl p-8 border-emerald-500/20 bg-night-panel/30 animate-in zoom-in-95 duration-300">
                    <div className="flex flex-col md:flex-row gap-8 items-start">
                        <div className="flex-1 w-full">
                            <div className="flex items-center justify-between mb-4">
                                <h4 className="text-lg font-bold text-slate-200 flex items-center gap-2">
                                    ...
                                    Alimentar IA em Lote
                                </h4>
                                <button onClick={() => setIsAdminUnlocked(false)} className="text-xs text-slate-500 hover:text-slate-300 underline">Bloquear</button>
                            </div>
                            
                            <p className="text-slate-400 text-sm leading-relaxed mb-4">
                            Cole abaixo uma lista de palavras (separadas por vírgula ou quebra de linha) para alimentar o banco de dados. A IA gera as definições e salva automaticamente.
                            </p>
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <div className="flex flex-wrap gap-3">
                                    ...
                                    <button 
                                        ...
                                        className="px-6 py-3 rounded-xl font-bold text-sm bg-night-panel border border-night-border text-slate-400 hover:text-slate-200 hover:border-slate-500 transition-all"
                                    >
                                        Usar Lista Padrão (20 Termos)
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Terminal Logs */}
                        <div className="w-full md:w-1/2 h-64 bg-black/40 rounded-lg border border-night-border p-4 font-mono text-xs overflow-y-auto shadow-inner">
